...
import com.briup.crm.service.ConstitudeService;
import com.briup.crm.service.CustomerService;

@Service
public class ConstituteServiceImpl implements ConstitudeService{
	@Autowired
	...
	public List<Contribution> findCustMarkup(int condition) {
		...
		if(condition==0) {//按等级
			...
			for(String level:levels) {
				//获得不同等对应的顾客数
				int size = custService.getCustByLevle(level);
				//获得每个等级所占百分比
