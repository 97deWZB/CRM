...
public class SalChanceServiceImpl implements SalChanceService{
	...
	public PageInfo<SalChance> findSalChance(int curPage, int size) {
		...
		List<SalChance> chanceList = chanceMapper.
				selectByExample(example);
		
		PageInfo<SalChance> chanceInfo =
				new PageInfo<SalChance>(chanceList);
		return chanceInfo;
	}
	...
	public PageInfo<SalChance> findSalChanceLike(int curPage, int size, String custName, String custRegion) {
		...
		andChcCustNameLike("%"+custName+"%").
		andChcAddrLike("%"+custRegion+"%");
		List<SalChance> chanceList = 
				chanceMapper.selectByExample(example);
	...
	@Override
	public void savaOrUpdate(SalChance chance) {
	...
	public SalChance findChanceById(long chanceId) {
		SalChance chance = 
				chanceMapper.selectByPrimaryKey(chanceId);
		return chance;
	}
	...
	@Override
	public PageInfo<SalChance> findChanceByUserId
				(int curPage,int size,String dueTo) {
	...
	@Override
	public PageInfo<SalChance> findChnaceByUserNameAndRegion(int curPage, int size, String dueTo, String region) {
		PageHelper.startPage(curPage, size);
		
		SalChanceExample example = new SalChanceExample();
		example.createCriteria().andChcDueToEqualTo(dueTo).
		andChcAddrEqualTo(region);
		List<SalChance> chancelist = 
				chanceMapper.selectByExample(example);
