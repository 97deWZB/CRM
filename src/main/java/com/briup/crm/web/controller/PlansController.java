...
public class PlansController {
	...
	public String findPlansByUserId(@PathVariable int curPage,HttpSession session) {
		...
		PageInfo<SalChance> chanceInfo = 
				chanceService.findChanceByUserId(curPage, 5, user.getUsrName());
	...
	public String findChanceByUserNameAndRegion(@PathVariable int curPage,String region,HttpSession session) {
		...
		PageInfo<SalChance> chanceInfo = 
				chanceService.findChnaceByUserNameAndRegion
				(curPage, 5, user.getUsrName(), region);
	...
	public String toPlanAdd(@PathVariable long chanceId,
			HttpSession session) {
	...
	public String toPlanDetail(@PathVariable long chanceId,HttpSession session) {
		SalChanceExtend chanceExtend = 
				chanceService.findChanceWithPlanById(chanceId);
		System.out.println("SOURce: "+chanceExtend.getChcSource());
		List<SalPlan> plans = chanceExtend.getPlans();
		for(SalPlan plan:plans) {
			System.out.println("-----"+plan.getPlaId()+" "+plan.getPlaResult());
		}
